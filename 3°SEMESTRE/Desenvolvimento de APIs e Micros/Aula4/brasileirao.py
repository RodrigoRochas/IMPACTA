...
def nome_campeao(dados):
    idCampeao = id_campeao(dados)
    nome = nome_do_time(dados, idCampeao)
...
def classificados_libertadores(dados):
    qtdLiberta = qtos_libertadores(dados)
    rs = ids_dos_melhor_classificados(dados, qtdLiberta)
...
def nomes_classificados_libertadores(dados):
    idClassificados = classificados_libertadores(dados)
    #print(dados2018['equipes'])
    equipes = []
    for idsClass in idClassificados:
        for i in dados['equipes'].keys():
            if i == idsClass:
                equipes.append(dados['equipes'][i]['nome-comum'])
...
def ids_dos_times_de_um_jogo(dados,id_jogo):
    ...
    return times['time1'],times['time2'] #assim a gente retorna as duas respostas em um unico return
...
def id_do_time(dados,nome_time):
    cont = 0
    tamanho_lista = len(dados['equipes'])
    while cont < tamanho_lista:
        equipe = dados['equipes']
        for i in equipe:
            if equipe[i]['nome-comum'] == nome_time:
                return equipe[i]['id']
        cont = cont + 1
...
def data_de_um_jogo(dados,id_jogo):
    dt_jogos = datas_de_jogo(dados)
    for data in dt_jogos:
...
def dicionario_id_estadio_e_nro_jogos(dados):
    ...
    dici = {}
    for i in jogos:
        id_estadio = jogos[i]['estadio_id']
        if(id_estadio in dici.keys()):
            adc = dici[id_estadio] + 1
            dici[id_estadio] = adc
        else:
            dici[id_estadio] = 1
    return dici
...
def ids_de_jogos_de_um_time(dados,time_id):
    ...
    for times in jogos:
        if jogos[times]['time1'] == time_id:
            lista.append(jogos[times])
